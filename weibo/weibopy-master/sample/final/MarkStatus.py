...
comments = pd.read_csv('./comments_final_marked.csv', index_col = 0)
statuses = pd.read_csv('./statuses_final_marked.csv', index_col = 0)

header = True
if os.path.exists('tmpt_file'):
    with open('tmpt_file', 'r') as f:
        begin = int(f.readline()) + 1
    header = False
else:
    begin = 0

tmpt_file = open('tmpt_file', 'w')
...
for sindex in statuses.index[begin:]:
    ...
    if (comments['sid'] == int(statuses.ix[sindex, 'sid'])).any():
        answer = raw_input("＊＊＊＊＊＊Show its comments?(yes[y]/no[n])＊＊＊＊＊＊: ")

        if answer[0] == 'y':
            for cindex in comments[comments['sid'] == statuses.ix[sindex, 'sid']].index:
                time.sleep(4)
                printComment(comments.ix[cindex])
    else:
        print "No comments Grabbed."

    if  statuses.ix[sindex, 'DD'] != None:
   
        print 'The DD of the status is {0}'.format(statuses.ix[sindex, 'DD'])
...
statuses.ix[begin:sindex].to_csv('./statuses-uc-dd.csv', mode='a', header=header)
